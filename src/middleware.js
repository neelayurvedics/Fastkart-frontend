...
export async function middleware(request) {
  ...
  // Safely fetch settings — guard against network errors or HTML error pages
  let settingData = null;
  try {
    const res = await fetch(process.env.URL + "/settings", requestOptions);
    const contentType = res.headers.get("content-type") || "";
    if (res.ok && contentType.includes("application/json")) {
      settingData = await res.json();
    } else {
      // Non-JSON or non-OK response (could be HTML error page). Log for debugging and continue.
      console.warn("/settings did not return JSON", res.status, contentType);
    }
  } catch (err) {
